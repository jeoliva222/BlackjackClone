...
public class GRoster {
	...
	public GRoster() {
		roster = new Opponent[] {
				...
				new Opponent("Oswald the Great", "He sees your hand.") {
					@Override
					public void doTurn() {
						GFrame frame = GFrame.getInstance();
						...
						// If player stayed and is winning, we must draw no matter what
						if(playerWinning && frame.playerStayed) {
							frame.dealAI();
							return;
						}
						
						// If we went over or if we're beating player and they stayed, hold for the turn
						if((score >= GFrame.numGoal) || (plrScore > GFrame.numGoal) ||
								(frame.playerStayed && (score > plrScore))) {
							frame.aiHold();
							return;
						} else {
							// Check for good and bad draw possibilities
